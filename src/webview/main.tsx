...
import {
  ReactFlow, ReactFlowProvider, Background, MiniMap, Handle, Position, ViewportPortal,
...
function BoardNode({ id, data, selected }: { id: string; data: BoardData; selected: boolean }) {
  ...
  // The compact stacked DIGEST card (purple shell + stackplates + foldrow + far-only LOD) shows only in the
  // FOLDED state. While preview-expanded, the representative reverts to its OWN real board card — clickable,
  // focusable, normal LOD — and the wrapping CollapseFrames overlay carries the group affordance + actions.
  const showCollapsedCard = isCollapsedGraph && !collapsePreviewOpen;
  ...
  const lod: Lod = showCollapsedCard ? 'far' : (zoomFarFar ? 'far-far' : (!zoomCompressed && (detailSet.has(id) || isFresh) ? 'detail' : 'far'));
  ...
  const turnBadge = showCollapsedCard
  ...
  // Folded card's "Expand" → opens the wrapping preview (reveals folded boards + reverts this rep to its real
  // card). Re-fold / Uncollapse live on the CollapseFrames header while previewing, not on the card.
  const expandCollapseBtn = showCollapsedCard ? (
    <button
      className="board__collapse-expand nodrag nopan"
      title="Preview collapsed history"
  ...
  return (
    ...
    <div className={`board-slot${showCollapsedCard ? ' board-slot--stacked' : ''}`}>
    ...
    <div
      className={`board lod-${lod} ${selected ? 'selected' : ''} ${inMergeCtx ? 'ctx-hl' : ''} ${revealed ? 'revealed' : ''} ${dimmed ? 'dimmed' : ''} ${needsAsk ? 'needs-ask' : ''} ${needsPerm ? 'needs-perm' : ''} ${data.unread ? 'unread' : ''} ${data.status} ${data.merged ? 'merged' : ''} ${data.compact ? 'compact' : ''} ${showCollapsedCard ? 'collapsed-graph' : ''} ${isArchived ? 'archived' : ''}`}
    >
      ...
      {lod !== 'far-far' && <TagChips tags={showCollapsedCard ? data.collapsedGraph?.tags : data.tags} />}
      <div className="board__head">
        ...
        <span className="board__title" title={showCollapsedCard ? (data.collapsedGraph?.summary || undefined) : undefined}>
          {showCollapsedCard
            ? collapsedGist
            : isDetail
        ...
        {showCollapsedCard ? null : compactBtn}
        {stopBtn}
      </div>
      ...
      {!isDetail || showCollapsedCard ? null : compacting ? (
...
// Padding (flow units) around the folded boards, plus a top band reserved for the frame header.
const COLLAPSE_FRAME_PAD = 28;
const COLLAPSE_FRAME_HEAD = 34;

// Overlay drawn around a collapse group while it is PREVIEW-EXPANDED (the user clicked Expand): a translucent
// box wrapping the revealed folded boards + the representative (which has reverted to its own real, clickable
// card — see BoardNode `showCollapsedCard`), with a header carrying the folded-history digest title + re-fold /
// uncollapse actions. Rendered via ViewportPortal so it lives in flow coordinates and pans/zooms with the
// canvas. Pure display: it reads node positions/measured sizes from App state and touches NO graph structure
// (not persisted, not in the nodes array, invisible to layout/merge/serialize). The box sits BEHIND the nodes
// (z-index:-1, pointer-events:none) so cards stay crisp and clickable; the header is a separate sibling that
// paints on top in the empty band above the topmost folded board.
function CollapseFrames({ nodes, onCollapse, onUncollapse }: {
  nodes: BoardNodeT[];
  ...
  const frames = useMemo(() => {
    ...
    const out: { id: string; x: number; y: number; w: number; h: number; title: string; count: number }[] = [];
    for (const n of nodes) {
      if (n.hidden || !n.data.collapsedGraph || !n.data.collapsePreviewExpanded) continue;
      let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity, any = false;
      for (const mid of [n.id, ...n.data.collapsedGraph.hiddenIds]) {
        const m = byId.get(mid);
        if (!m || m.hidden) continue;
        const w = m.measured?.width ?? 320, h = m.measured?.height ?? 200;
      ...
      out.push({
        id: n.id,
        x: minX - COLLAPSE_FRAME_PAD,
        y: minY - COLLAPSE_FRAME_PAD - COLLAPSE_FRAME_HEAD,
        w: (maxX - minX) + COLLAPSE_FRAME_PAD * 2,
        h: (maxY - minY) + COLLAPSE_FRAME_PAD * 2 + COLLAPSE_FRAME_HEAD,
        title: clampLabel(n.data.collapsedGraph.miniSummary || summaryHeadline(n.data.collapsedGraph.summary ?? '') || 'Collapsed history'),
        count: n.data.collapsedGraph.hiddenIds.length + 1,
      });
    }
    return out;
  }, [nodes]);
    ...
    <ViewportPortal>
      {frames.map((f) => (
        <React.Fragment key={f.id}>
          <div className="collapse-frame__box" aria-hidden="true" style={{ transform: `translate(${f.x}px, ${f.y}px)`, width: f.w, height: f.h }} />
          <div
            className="collapse-frame__header nodrag nopan"
            style={{ transform: `translate(${f.x + 8}px, ${f.y + 6}px)`, maxWidth: Math.max(120, f.w - 16) }}
            onClick={(e) => e.stopPropagation()}
            onPointerDown={(e) => e.stopPropagation()}
          >
            <span className="collapse-frame__title" title={f.title}>{f.title}</span>
            <span className="collapse-frame__count">{f.count} folded</span>
            <button className="collapse-frame__btn nodrag nopan" title="Re-fold this collapsed history into the digest card" onClick={(e) => { e.stopPropagation(); onCollapse(f.id); }}>Collapse</button>
            <button className="collapse-frame__btn nodrag nopan" title="Permanently remove this collapse (auto-collapse may fold it again later)" onClick={(e) => { e.stopPropagation(); onUncollapse(f.id); }}>Uncollapse</button>
          </div>
        </React.Fragment>
      ))}
    </ViewportPortal>
...
function App() {
  ...
  const wideIds = useMemo(() => {
    ...
    for (const n of nodes) {
      ...
      // A folded representative is never detail-wide, but a PREVIEW-expanded one reverts to its real card and
      // can grow to detail width like any board — let it into the set so the centering relayout tracks it.
      if ((!n.data.collapsedGraph || n.data.collapsePreviewExpanded) && (detailIds.has(n.id) || fresh)) s.add(n.id);
    }
    return s;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Wrapping frame(s) around any preview-expanded collapse group (flow-coord overlay; pure display). */}
        <CollapseFrames nodes={nodes} onCollapse={expandCollapsed} onUncollapse={uncollapseCollapsed} />
        <MiniMap
