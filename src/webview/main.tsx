...
const CollapseCtx = React.createContext<{ expand: (id: string) => void; collapse: (id: string) => void; collapsible: Set<string> }>({ expand: () => {}, collapse: () => {}, collapsible: new Set() });
...
function BoardNode({ id, data, selected }: { id: string; data: BoardData; selected: boolean }) {
  ...
  // This board can fold its prior visible history into a hidden subgraph (a non-empty collapse plan) → it
  // shows a per-board "collapse history" button in detail LOD (App computes the set over the whole graph).
  const canCollapse = collapseCtx.collapsible.has(id);
  ...
  // Per-board collapse: fold this board's earlier visible history into a hidden subgraph. Lives on the board
  // (detail LOD only) next to the compact 🗜 button — icon-only, shown only when there's history to fold.
  const collapseBtn = isDetail && !isCollapsedGraph && canCollapse ? (
    <button
      className="board__collapse nodrag nopan"
      title="Collapse history: fold this board's earlier history into a hidden, expandable subgraph"
      onClick={(e) => { e.stopPropagation(); collapseCtx.collapse(id); }}
    >⊟</button>
  ) : null;
  ...
  return (
    ...
    >
      ...
      <div className="board__head">
        ...
        {/* Collapsed node: a small chip showing how many boards are folded inside (the gist carries meaning). */}
        {isCollapsedGraph && <span className="board__foldcount" title={`${collapsedCount} boards folded here`}>{collapsedCount}</span>}
        ...
        {isCollapsedGraph ? expandCollapseBtn : (<>{collapseBtn}{compactBtn}</>)}
        {stopBtn}
      </div>
...
function App() {
  ...
  // Per-board collapse affordance: ids whose prior visible history can be folded into a hidden subgraph (a
  // non-empty collapse plan for that board alone). Each BoardNode shows its own "collapse history" button (in
  // detail LOD) when its id is here. Pre-filtered to boards that have a parent and aren't already a collapsed
  // representative, so roots / dead-ends are skipped cheaply before the per-board plan walk.
  const collapsibleIds = useMemo(() => {
    const hasParent = new Set(edges.filter((e) => !e.hidden).map((e) => e.target));
    const s = new Set<string>();
    for (const n of nodes) {
      if (n.hidden || n.data.collapsedGraph || !hasParent.has(n.id)) continue;
      if (planCollapseSelection(nodes, edges, [n.id]).length > 0) s.add(n.id);
    }
    return s;
  }, [nodes, edges]);
  ...
  // Fold ONE board's prior visible history into a hidden subgraph (the per-board "collapse history" button).
  // Collapsing siblings one at a time is safe: planCollapseSelection's guard keeps the target at the first
  // shared ancestor when hiding it would detach a still-visible sibling branch.
  const collapseHistory = useCallback((id: string) => {
    const curNodes = nodesRef.current;
    const curEdges = edgesRef.current;
    const collapsed = collapseSelection(curNodes, curEdges, [id]);
  ...
  }, [patch, patchTurn, queueFinishFields, queueStartFields, restoreGraph, seedRoot, wantsAttention, tryReveal]);
